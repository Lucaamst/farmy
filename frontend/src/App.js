...
function SuperAdminDashboard() {
  ...
  const [loading, setLoading] = useState(true);
  const [newCompany, setNewCompany] = useState({ name: '', admin_username: '', admin_password: '', phone_number: '', sms_sender_name: '' });
        ...
        <Dialog open={show2FADialog} onOpenChange={setShow2FADialog}>
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label htmlFor="otpCode">Inserisci il codice a 6 cifre</Label>
                <Input
                  id="otpCode"
                  type="text"
                  maxLength={6}
...
function CompanyAdminDashboard() {
  ...
  const [newOrder, setNewOrder] = useState({ customer_name: '', delivery_address: '', phone_number: '', reference_number: '', customer_id: '', requires_signature: false, send_sms: true, priority: 'normal', order_notes: '' });
  ...
  const createOrder = async (e) => {
    ...
    try {
      ...
      setNewOrder({ customer_name: '', delivery_address: '', phone_number: '', reference_number: '', customer_id: '', requires_signature: false, send_sms: true });
      setCustomerSearch('');
  ...
  const downloadCourierStatsPDF = async (courier, stats, history) => {
    ...
    let htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Statistiche ${courierName}</title>
        <style>
          @page { margin: 2cm; }
          body { 
            font-family: Arial, sans-serif; 
      ...
      <body>
        ...
        <div class="stats-box">
          ...
          <div class="stats-grid">
            ...
            <div class="stat-item">
              <div class="stat-value">${stats.monthly.length}</div>
              <div class="stat-label">Mesi Attivi</div>
            </div>
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              <th>Mese</th>
              <th style="text-align: center;">Numero Consegne</th>
            </tr>
          </thead>
          <tbody>
            ${stats.monthly.map(m => `
              <tr>
                <td style="text-transform: capitalize;">${m.month}</td>
                <td style="text-align: center;"><span class="badge badge-green">${m.count} consegne</span></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-xs sm:text-sm">{t.status}</TableHead>
                        <TableHead className="text-xs sm:text-sm">Firma</TableHead>
