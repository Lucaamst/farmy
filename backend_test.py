...
class DeliveryManagementAPITester:
    ...
    def test_order_creation_with_signature_requirement(self):
        ...
        signature_order_id = None
        if success1 and 'order' in response1:
            signature_order_id = response1['order']['id']
        ...
        no_signature_order_id = None
        if success2 and 'order' in response2:
            no_signature_order_id = response2['order']['id']
        ...
        # Test 3: Verify orders are saved correctly in database by retrieving them
        success3 = True
        if signature_order_id:
            ...
            if success3a and isinstance(response3a, list):
                sig_order_found = any(
                    order.get('id') == signature_order_id and order.get('requires_signature') == True
                    for order in response3a
                )
                no_sig_order_found = any(
                    order.get('id') == no_signature_order_id and order.get('requires_signature') == False
                    for order in response3a
                )
                success3 = sig_order_found and no_sig_order_found
            else:
                success3 = False
        
        overall_success = success1 and success2 and requires_sig_correct and no_sig_correct and success3
        
        if overall_success:
            return self.log_test("Order Creation with Signature Requirement", True, 
                               f"- Order with signature: ✅, Order without signature: ✅, Database verification: ✅")
        else:
            details = f"- With sig: {success1}, Without sig: {success2}, Sig field correct: {requires_sig_correct}, No sig field correct: {no_sig_correct}, DB verification: {success3}"
    ...
    def test_delivery_completion_with_signature(self):
        ...
        # Ensure we have orders and courier for testing
        if 'signature_order' not in self.test_data or 'no_signature_order' not in self.test_data:
            return self.log_test("Delivery Completion with Signature", False, "- Missing test orders")
        
        if 'courier_id' not in self.test_data or 'courier' not in self.tokens:
            return self.log_test("Delivery Completion with Signature", False, "- Missing courier data")
        ...
        # Create another signature-required order for this test
        test_order_data = {
        ...
        # Test 4: Complete signature-required order with signature_skipped flag
        if success3a and 'order' in response3a:
        ...
        if overall_success:
            ...
            details = f"- With sig: {success1} ({status1}), Without sig: {success2} ({status2}), Missing blocked: {success3} ({status3}), Skipped: {success4}"
    ...
    def test_pdf_generation(self):
        ...
        if 'signature_order' not in self.test_data:
            return self.log_test("PDF Generation", False, "- No signature order available")
        ...
        if 'no_signature_order' not in self.test_data:
            success2 = False
        ...
        # Test 5: Test role-based access (only company admin should access PDF)
        success5, status5, response5 = self.make_request(
            'GET', f'orders/{signature_order_id}/delivery-confirmation-pdf',
            token=self.tokens.get('courier'),
            expected_status=403
        ...
        overall_success = success1 and pdf_correct and success2 and pdf_no_sig_correct and success3 and success4 and success5
        
        if overall_success:
            return self.log_test("PDF Generation", True, 
                               f"- With signature PDF: ✅, Without signature PDF: ✅, Non-delivered blocked: ✅, Non-existent blocked: ✅, Role access: ✅")
        else:
            details = f"- With sig PDF: {success1}/{pdf_correct}, Without sig PDF: {success2}/{pdf_no_sig_correct}, Non-delivered: {success3} ({status3}), Non-existent: {success4} ({status4}), Role access: {success5} ({status5})"
